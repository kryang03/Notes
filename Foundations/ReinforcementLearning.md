...
其中 $\phi(q)$ 是接触距离函数（即 [[ComputationalGeometry#4. 有向距离场 (SDF)：连续优化的基石|SDF]]）。若在全空间 $\mathbb{R}^n$ 上加各向同性高斯噪声去探索，会立刻撞上两种失效：**穿透**（指令把手指压进笔内部，物理引擎产生巨大排斥力、仿真爆炸）与**脱离**（不慎断开接触、笔掉落）。【WHY?】
...
DQN 与一切含 $\max$ 的更新都背着一道诅咒。先把它讲成定理，再讲成解药。【WHY?】
...
>
> **对灵巧操作，$T$ 与 $A$ 是主瓶颈**——接触非线性（[[ContactMechanics]]）+ 执行器非理想（[[Dynamics#8. 腱驱动与冗余动力学：真实灵巧手的传动|腱驱动]]/减速器）共同构成 gap 核心。**先分清是哪一类 gap，再选药**，否则乱投医。
