...
## 0. 理论大厦构建路线：从“会看会说”到“能安全改变世界”

具身智能的理论骨架不能停留在“VLA = 视觉 + 语言 + 动作”的定义层面。它应当被理解为一个逐层收紧的闭环系统：先把开放世界语义压缩成可执行任务，再把任务落到连续动作，再用底层控制和真实反馈保证物理安全。

| 层级 | 核心问题 | 理论工具 | 灵巧操作中的检查点 |
|:--|:--|:--|:--|
| 任务语义层 | 人类指令如何变成可验证目标？ | 语言 grounding、任务图、前置/后置条件 | “转笔一圈”必须拆成发力、旋转、换指、接住等相位 |
| 空间表征层 | 语义目标在哪里、与手如何相对？ | 3D 表征、点云、SDF、物体中心坐标 | 只识别“笔”不够，必须定位接触边、轴线和可推方向 |
| 动作生成层 | 连续动作如何表达多模态方案？ | AR token、Diffusion、Flow Matching、action chunk | 同一状态可左绕/右绕，均值动作常常物理无效 |
| 控制执行层 | 大模型动作如何变成稳定力学交互？ | 阻抗/导纳、MPC、安全过滤、频率分层 | VLA 不应直接接电机；必须有低层闭环与限幅 |
| 数据飞轮层 | 系统如何从失败中持续变强？ | IL → offline RL → online RL、人类校正、world model | 真机失败要回流成接触/执行器/策略的可诊断数据 |

> [!important] Foundation 级判断标准
> 一个 Embodied AI 系统只有在能回答“语义目标如何约束物理状态、动作表示如何尊重执行器带宽、反馈如何修正分布外接触”时，才真正进入机器人理论大厦。否则它只是把 VLM 接到 action head 的工程拼接。
