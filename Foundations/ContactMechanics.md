...
## 0. 理论大厦构建路线：从接触点到可计算力学

接触力学的理论大厦有五层，每一层都回答一个更具体的问题：

| 层级 | 要回答的问题 | 关键对象 | 下游依赖 |
|:--|:--|:--|:--|
| 接触几何 | 接触点在哪里、法向和曲率如何变化？ | 高斯标架、曲率张量、Montana 方程 | [[ComputationalGeometry]] 的 SDF/最近点查询 |
| 接触运动学 | 接触点如何在两曲面上滚动/滑动？ | 接触雅可比、非完整约束 | [[ControlTheory]] 的力/位混合与接触状态机 |
| 接触静力学 | 接触力能否抵抗任意扰动？ | 抓取矩阵、摩擦锥、力闭合、内力 | 抓取合成、contact map、grasp quality |
| 接触动力学 | 接触力如何与速度跳变和不可穿透耦合？ | LCP、冲量、互补条件 | [[Dynamics]] 的接触时间步进与仿真器 |
| 可微/学习层 | 如何让接触进入梯度优化或策略学习？ | 平滑化、隐式微分、随机化 | [[Optimization]] 的 CITO、[[ReinforcementLearning]] 的 contact-rich policy |

> [!important] 细颗粒度检查标准
> 任何接触概念都要同时说明“几何约束是什么、允许哪些速度、能传哪些力、求解器如何数值实现、对灵巧手策略有什么风险”。只写“摩擦锥/力闭合定义”是不够的。
...
**手部雅可比 (Hand Jacobian, $J_h$)** 将关节速度 $\dot{q}$ 映射到手指末端的高斯标架速度。而**抓取矩阵 (Grasp Matrix, $G$)** 则描述了接触点处的力如何传递到物体的质心（COM）产生合力和合力矩 。这二者不是同一个方向的映射，而是通过虚功原理形成对偶。

$$V_{contact} = J_h \dot{q}$$

$$W_{object} = G f_c, \qquad \tau_q = J_h^T f_c$$

其中 $f_c$ 是所有接触力的堆叠，$W_{object}$ 是物体旋量（Wrench），$\tau_q$ 是关节力矩。重要的是，抓取矩阵 $G$ 的结构直接取决于所采用的**接触模型**（点接触、硬指、软指）。对于软指接触，$G$ 矩阵不仅包含力的传递块，还必须包含力矩传递块，这增加了系统的可控自由度，使得某些在点接触模型下不可控的操作（如原地旋转物体）成为可能。

> [!tip] 对偶关系的物理含义
> $J_h$ 回答“关节运动会让接触点怎么动”，$J_h^T$ 回答“接触力会在关节上造成多大负载”；$G$ 回答“接触力会对物体造成什么 wrench”。多指操作的本质就是同时选择 $\dot q$ 和 $f_c$，让运动约束与力约束都可行。
