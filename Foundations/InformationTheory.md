...
把物体表面建成隐函数 $f(x)=0$：空间点 $x$ 的函数值 $f(x)$ = 到表面的有向距离 (SDF，见 [[ComputationalGeometry#4. 有向距离场 (SDF)：连续操作优化的基石|SDF]])。接触点 $y=0$、自由空间 $y>0$、内部 $y<0$。GP 给函数上的分布 $f(x)\sim\mathcal{GP}(m,k)$，对查询点输出高斯 $\mathcal N(\mu_*,\sigma_*^2)$——**均值是形状估计、方差是该估计的把握**，天然契合主动探索。核函数 $k(x,x')$（RBF/Matérn）的长度尺度对应物体的**特征尺度**（纹理粗糙度/几何特征大小）：太大则过度平滑掉钥匙齿，太小则泛化差（认为相邻两点无关）。
