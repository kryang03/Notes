...
然而，本报告关注的**动态非抓取操作（DNPM）**颠覆了这一假设。
$$\mathbf{M}(\mathbf{q})\ddot{\mathbf{q}} + \mathbf{C}(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}}+\mathbf{g}(\mathbf{q}) = \mathbf{J}^T \mathbf{F}_{ext}$$根据的界定，DNPM系统的核心特征在于：**系统不总能直接施加力抗衡重力**。这意味着在任务的关键阶段，控制权是欠驱动的（Underactuated）。机器人必须“借力打力”，通过主动引入高动能，使系统进入一个由惯性主导的状态空间。

**动力学因果长链条解析：**

1. **主动发力（Active Energy Injection）：** 机器人（UR5或灵巧手）在初始阶段施加瞬时高扭矩或快速运动，向物体注入动能。
2. **进入高惯性状态（High-Inertia State）：** 物体获得极高的线速度或角速度，此时惯性项在动力学方程中不可忽视。
...
4. **物理演化（Physical Evolution）：** 物体在非完全约束的状态下，沿着预期的动力学轨迹演变（如飞行、翻滚、滑移）并完成目标轨迹。
...
#### 任务 1.1：蝴蝶刀翻转 (Balisong Flipping) - [推荐但高难]
...
#### 任务 1.3：Zippo打火机花式开盖 (Zippo Tricks)
...
#### 任务 3.2：托盘平衡与运输 (Non-Prehensile Tray Balancing/Waiter Task) - [核心推荐]
...
2. **进阶级（主要攻关）：** **颠锅翻炒（Wok Tossing）；Thumbaround**。这是一个完美的**臂-手协作**任务。仿真中使用刚体粒子近似食物非常成熟，甚至可以直接用一个刚体代表薄饼/煎蛋。该任务既展示了长链条动力学（发力-飞行-接住），又具备极强的日常生活应用背景，且容错率略高于蝴蝶刀。Thumbaround的“Snap”动作是对系统瞬时爆发力和时序控制的极致考验。
3. **挑战级（高动态）：** **蝴蝶刀（Balisong）**。这类任务专注于铰接物体的瞬态动力学，是展示灵巧手“指尖功夫”的绝佳场景。
