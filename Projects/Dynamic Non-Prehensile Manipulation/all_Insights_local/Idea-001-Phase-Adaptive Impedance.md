...
**局限 1：固定 PD 的力矩表达上限严重受限。** 在 Thumbaround 的训练后力矩图中（见 RSS26.pdf 第15页），关节实际位移极小，策略的 "位置控制" 实质上是在通过 PD 间接输出力矩。然而，固定 $K_p$, $K_d$ 无法表达 "先硬后软"、"振荡式" 等动力学任务所需的力矩模式——这正是 [[ControlTheory#3.1.1 从 PID 到计算力矩：精确线性化的诱惑与局限]] 中指出的 PD 的本质缺陷。
...
- [[ControlTheory#3.1.1 从 PID 到计算力矩：精确线性化的诱惑与局限]] — 解释为什么固定 PD 在动态任务中力矩表达受限
