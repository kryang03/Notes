...
> [!abstract] 核心贡献
> MoDem-V2 关注真实机器人 visuomotor world model：从视觉和动作中学习可用于真实 manipulation 的预测/规划表示。
...
它在当前知识库中的角色是：WMTS 可把 MoDem-V2 作为视觉接口参考，但灵巧手还要增加 tactile/proprioceptive latent，否则看不见的接触会被模型猜错。

## 1. 问题设定与动机

### 1.1 一句话核心
真实操作中的视觉观测高维且部分可观测，动作后果受接触和遮挡影响；只学 policy 很难复用环境动力学知识。
...
### 1.4 Delta 分析
把视觉表征、动作条件 dynamics 和控制目标放进同一个 world model，可以提升少样本真实操作效率。
...
### 2.3 论文核心机制无跳步推导
- 收集真实机器人视觉-动作轨迹。
- 学习 latent state 和 action-conditioned transition。
- 用模型辅助 planning、policy learning 或 data augmentation。
...
2. 方法引入结构性假设：把视觉表征、动作条件 dynamics 和控制目标放进同一个 world model，可以提升少样本真实操作效率。
...
### 3.1 PDF 结构线索
- I. I NTRODUCTION
- 1 Patrick Lancaster, Aravind Rajeswaran, and Vikash Kumar are

### 3.2 关键结果与证据
关注真实机器人任务成功率、所需数据分钟数、视觉预测质量和 downstream control gain。

- PDF 线索：strumented real-world environments must perceive the world
- PDF 线索：uninstrumented real world. Building on the latest algorithmic
- PDF 线索：the real world. We identify key ingredients for leveraging
- PDF 线索：demonstrations in model learning while respecting real-world
- PDF 线索：nipulation problems in both simulation and the real world. To Fig. 1. We use MoDem-V2 to train the robot on four contact-rich
- PDF 线索：the best of our knowledge, our work presents the first successful manipulation tasks. These tasks cover a wide range of manipulation skills,

### 3.3 Ablation 因果链
没有 world model -> 数据效率低；没有视觉条件 -> 无法处理物体状态变化；没有真实微调 -> sim/离线偏差保留。
...
### 4.3 什么时候会失效
视觉 world model 对遮挡和细微接触不敏感；不能单独承担转笔接触 phase 判定。
...
WMTS 可把 MoDem-V2 作为视觉接口参考，但灵巧手还要增加 tactile/proprioceptive latent，否则看不见的接触会被模型猜错。
...
model-based RL / imagination rollout。这篇论文提供的是一个可迁移的结构化 bias：它把 真实操作中的视觉观测高维且部分可观测，动作后果受接触和遮挡影响；只学 policy 很难复用环境动力学知识。 转化为可建模、可采样或可约束的问题。

### 与 [[StochasticProcess]] 的联系
latent transition and uncertainty。这篇论文提供的是一个可迁移的结构化 bias：它把 真实操作中的视觉观测高维且部分可观测，动作后果受接触和遮挡影响；只学 policy 很难复用环境动力学知识。 转化为可建模、可采样或可约束的问题。

### 与 [[EmbodiedAI]] 的联系
robot learning loop。这篇论文提供的是一个可迁移的结构化 bias：它把 真实操作中的视觉观测高维且部分可观测，动作后果受接触和遮挡影响；只学 policy 很难复用环境动力学知识。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[MoDem-V2- Visuo-Motor World Models for Real-World Robot Manipulation.pdf]]
- 项目入口：[[Final_WMTS]]
