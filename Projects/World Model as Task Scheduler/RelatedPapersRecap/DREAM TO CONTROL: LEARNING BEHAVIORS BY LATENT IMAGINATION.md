...
> [!abstract] 核心贡献
> Dreamer 的核心 insight 是：先学一个 RSSM latent world model，再在 latent imagination 里训练 actor-critic，把真实交互样本效率问题转化为模型内 rollout 的优化问题。
...
它在当前知识库中的角色是：WMTS 的 scheduler 可以借鉴 Dreamer：在 actuator+rigid WM 中 rollout candidate task/action chunks，再用 value 或 constraint 选择真实试验。

## 1. 问题设定与动机

### 1.1 一句话核心
Model-free RL 需要大量真实交互；像素级模型预测又太难且不一定服务控制。
...
### 1.4 Delta 分析
只要 latent state 保留 reward、discount 和可控 dynamics，策略优化不需要完整重构真实世界，只需要在可预测 latent 中做 imagination。
...
### 2.3 论文核心机制无跳步推导
- 从 replay buffer 训练 encoder、RSSM transition、reward/discount/decoder。
- 从 posterior state 出发 rollout prior dynamics，得到 imagined latent trajectory。
- actor 最大化 imagined return，critic 学 latent value，真实环境只用于补充数据。
...
2. 方法引入结构性假设：只要 latent state 保留 reward、discount 和可控 dynamics，策略优化不需要完整重构真实世界，只需要在可预测 latent 中做 imagination。
...
### 3.2 关键结果与证据
看 DMControl 等连续控制任务上的 sample efficiency；关键不是像素质量，而是 imagined rollout 对 value/policy gradient 是否有用。

- PDF 线索：agent that solves long-horizon tasks from images purely by latent imagination.
- PDF 线索：world model. On 20 challenging visual control tasks, Dreamer exceeds existing
- PDF 线索：and 3D environments. Several of these tasks could previously not be solved through world models.
- PDF 线索：maximizes the expected sum of rewards Ep t=1 rt . Figure 2 shows a selection of our tasks.
- PDF 线索：a value model, an ablation we compare to in our experiments. VN estimates rewards beyond k steps
- PDF 线索：enables Dreamer to solve long-horizon tasks while being robust to the imagination horizon. The

### 3.3 Ablation 因果链
去掉 latent imagination -> 退回 model-free 样本效率；去掉 recurrent stochastic state -> 长期不确定性和 partial observability 表达不足。
...
### 4.3 什么时候会失效
Dreamer 对模型偏差敏感；灵巧手 contact discontinuity 会让 latent rollout 很快偏离真实，必须限制 horizon 或加入 uncertainty penalty。
...
WMTS 的 scheduler 可以借鉴 Dreamer：在 actuator+rigid WM 中 rollout candidate task/action chunks，再用 value 或 constraint 选择真实试验。
...
model-based RL / imagination rollout。这篇论文提供的是一个可迁移的结构化 bias：它把 Model-free RL 需要大量真实交互；像素级模型预测又太难且不一定服务控制。 转化为可建模、可采样或可约束的问题。

### 与 [[StochasticProcess]] 的联系
latent transition and uncertainty。这篇论文提供的是一个可迁移的结构化 bias：它把 Model-free RL 需要大量真实交互；像素级模型预测又太难且不一定服务控制。 转化为可建模、可采样或可约束的问题。

### 与 [[EmbodiedAI]] 的联系
robot learning loop。这篇论文提供的是一个可迁移的结构化 bias：它把 Model-free RL 需要大量真实交互；像素级模型预测又太难且不一定服务控制。 转化为可建模、可采样或可约束的问题。
