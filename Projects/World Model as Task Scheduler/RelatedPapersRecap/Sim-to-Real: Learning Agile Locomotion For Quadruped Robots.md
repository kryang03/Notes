...
> [!abstract] 核心贡献
> 这篇经典工作展示大规模仿真+域随机化可以训练可转移四足 locomotion policy，是后来 actuator model 和 sim-to-real dexterity 的前置范式。
...
它在当前知识库中的角色是：WMTS 需要从“随机化覆盖真实”升级为“模型解释真实”：对 L25 执行器，随机化只是临时办法，最终要学 actuator residual。

## 1. 问题设定与动机

### 1.1 一句话核心
真实机器人训练成本高且易损坏；仿真策略又会 overfit 精确物理参数。
...
### 1.4 Delta 分析
随机化质量、摩擦、延迟、传感噪声和地形，可以让策略学到覆盖真实系统的鲁棒控制规律。
...
2. 方法引入结构性假设：随机化质量、摩擦、延迟、传感噪声和地形，可以让策略学到覆盖真实系统的鲁棒控制规律。
...
- V. NARROWING THE R EALITY G AP

### 3.2 关键结果与证据
关注真机速度、稳定性、地形泛化和随机化消融。

- PDF 线索：After learning in simulation, a quadruped robot can successfully
- PDF 线索：I. I NTRODUCTION to locomotion tasks due to the difficulties of automatically
- PDF 线索：control an under-actuated robot performing highly dynamic the robot. Thus for locomotion tasks, learning in simulation is
- PDF 线索：performs poorly in the real world. This reality gap [7, 8] of the learned policy sometimes is preferred. Our learning
- PDF 线索：in locomotion tasks. When a robot performs agile motion with learned gait close to the reference while, at the same time,
- PDF 线索：system. While this has been successfully demonstrated in gap, we experiment with three approaches to increase the

### 3.3 Ablation 因果链
固定参数仿真 -> 真实迁移失败；随机化范围过窄 -> 不覆盖真实；过宽 -> 学习困难。
...
### 4.3 什么时候会失效
四足 gait 的重复接触结构比手内操作更规则；直接照搬 DR 范围会低估多指接触组合爆炸。
...
WMTS 需要从“随机化覆盖真实”升级为“模型解释真实”：对 L25 执行器，随机化只是临时办法，最终要学 actuator residual。
...
### 与 [[Dynamics]] 的联系
whole-body/contact dynamics。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人训练成本高且易损坏；仿真策略又会 overfit 精确物理参数。 转化为可建模、可采样或可约束的问题。

### 与 [[ControlTheory]] 的联系
low-level tracking and stability。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人训练成本高且易损坏；仿真策略又会 overfit 精确物理参数。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
sim-to-real policy learning。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人训练成本高且易损坏；仿真策略又会 overfit 精确物理参数。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[Sim-to-Real: Learning Agile Locomotion For Quadruped Robots.pdf]]
- 项目入口：[[Final_WMTS]]
