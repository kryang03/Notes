...
> [!abstract] 核心贡献
> DexterityGen 的核心是把灵巧操作控制器做成 foundation controller：用大规模任务/物体/动作覆盖学一个可复用的低层 dexterity prior。
...
它在当前知识库中的角色是：WMTS 可以让 Task Scheduler 输出 controller condition，而不是直接输出低层动作；这会把长 horizon 规划和短 horizon 接触稳定分开。

## 1. 问题设定与动机

### 1.1 一句话核心
单任务 dexterous policy 通常学到的是任务特化的接触技巧，换物体、换目标或换动作族就失效。
...
### 1.4 Delta 分析
通过大规模多任务训练和统一动作接口，可以让策略学到跨任务复用的接触调节、手形协同和稳定化反射。
...
### 2.3 论文核心机制无跳步推导
- 构造多任务灵巧操作数据或仿真任务族。
- 训练统一 controller 接收目标、物体/手状态和任务条件。
- 在下游任务中用 prompt/goal/adapter 调用已有 dexterity prior，而不是从零学习接触。
...
2. 方法引入结构性假设：通过大规模多任务训练和统一动作接口，可以让策略学到跨任务复用的接触调节、手形协同和稳定化反射。
...
### 3.1 PDF 结构线索
- 1 BAIR, UC Berkeley          2 FAIR at Meta
- B. Simulated Experiments
- D. Real World Results
- A. DexGen Training Pipeline

### 3.2 关键结果与证据
关注跨物体、跨技能、跨目标的成功率，以及 foundation controller 对少样本适应的提升。

- PDF 线索：eration (for imitation learning) and sim-to-real reinforcement controller. In the real world, we use human teleoperation as a
- PDF 线索：without touch feedback. The second RL-based approach struggles real world, demonstrating that it is a general-purpose controller
- PDF 线索：gineering on complex tasks. Our key insight is that RL is effective significantly improves stability by 10-100x measured as duration
- PDF 线索：at learning low-level motion primitives, while humans excel at of holding objects across diverse tasks. Notably, with DexGen
- PDF 线索：providing coarse motion commands for complex, long-horizon we demonstrate unprecedented dexterous skills including diverse
- PDF 线索：tasks. Therefore, the optimal solution might be a combination of object reorientation and dexterous tool use such as pen, syringe,

### 3.3 Ablation 因果链
去掉多任务覆盖或统一条件化 -> controller 退化为任务专才 -> 因为接触模式没有被迫形成可组合表示。
...
### 4.3 什么时候会失效
foundation controller 若训练物理域和真机执行器差距过大，会稳定地产生“看似合理但电机不可执行”的动作。
...
WMTS 可以让 Task Scheduler 输出 controller condition，而不是直接输出低层动作；这会把长 horizon 规划和短 horizon 接触稳定分开。
...
contact mode, friction, grasp stability。这篇论文提供的是一个可迁移的结构化 bias：它把 单任务 dexterous policy 通常学到的是任务特化的接触技巧，换物体、换目标或换动作族就失效。 转化为可建模、可采样或可约束的问题。

### 与 [[Dynamics]] 的联系
hand-object rigid body dynamics。这篇论文提供的是一个可迁移的结构化 bias：它把 单任务 dexterous policy 通常学到的是任务特化的接触技巧，换物体、换目标或换动作族就失效。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
policy learning under contact。这篇论文提供的是一个可迁移的结构化 bias：它把 单任务 dexterous policy 通常学到的是任务特化的接触技巧，换物体、换目标或换动作族就失效。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[DEXTERITYGEN- Foundation Controller for Unprecedented Dexterity.pdf]]
- 项目入口：[[Final_WMTS]]
