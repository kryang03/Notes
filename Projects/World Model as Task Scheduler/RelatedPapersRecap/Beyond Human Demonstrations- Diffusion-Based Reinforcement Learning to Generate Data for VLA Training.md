...
> [!abstract] 核心贡献
> 这篇论文把 diffusion policy 从“模仿人类示范”推进到“用 RL 生成超出人类示范分布的数据”，核心瓶颈是 VLA 训练数据质量受限于人类轨迹的覆盖和次优性。
...
它在当前知识库中的角色是：WMTS 可借鉴为 world-model-guided diffusion：先用 DP 学手内操作动作先验，再用 WM reward/constraint 在 latent imagination 中偏移生成分布。

## 1. 问题设定与动机

### 1.1 一句话核心
VLA/模仿学习依赖人类 demonstrations，但人类示范常常覆盖窄、动作保守、失败边界少；直接 RL 又容易在高维动作空间中不稳定。
...
### 1.4 Delta 分析
Diffusion policy 提供稳定的动作先验，RL 只需要在这个先验周围优化高回报轨迹，就能产生比原始示范更适合训练 VLA 的数据。
...
### 2.3 论文核心机制无跳步推导
- 先用示范训练条件 diffusion policy，得到多模态动作分布。
- 把去噪采样过程作为可优化策略或数据生成器，在环境 reward 下偏移采样分布。
- 筛选/混合生成轨迹，用于 VLA 或下游策略训练。
...
2. 方法引入结构性假设：Diffusion policy 提供稳定的动作先验，RL 只需要在这个先验周围优化高回报轨迹，就能产生比原始示范更适合训练 VLA 的数据。
...
### 3.1 PDF 结构线索
- I. I NTRODUCTION
- A. Data Generation for VLA Models

### 3.2 关键结果与证据
关注生成数据带来的下游 VLA 成功率提升、示范外任务覆盖、以及 reward 优化是否牺牲动作自然性。

- PDF 线索：130 Tasks Diverse Settings Generalization to Unseen Tasks
- PDF 线索：≈ 6k Trajectories Different goals Training Tasks Unseen Tasks
- PDF 线索：the LIBERO benchmark across diverse tasks and settings, demonstrating that our RL-generated trajectories provide effective
- PDF 线索：strong generalization across tasks and embodiments; however, diffusion policy optimization algorithm to generate high-quality
- PDF 线索：algorithms often struggle on long-horizon manipulation tasks regularization of the iterative denoising process to yield smooth
- PDF 线索：Research Asia. manipulation tasks, and show that the generated trajectories are

### 3.3 Ablation 因果链
只用 BC 数据 -> 覆盖受限；只用 RL -> 探索和动作先验不足；diffusion+RL 有效说明“生成先验”和“任务回报”承担不同角色。
...
### 4.3 什么时候会失效
若 world model reward 错误，RL 会把 diffusion 先验推向模型漏洞；必须保留真实验证和约束过滤。
...
WMTS 可借鉴为 world-model-guided diffusion：先用 DP 学手内操作动作先验，再用 WM reward/constraint 在 latent imagination 中偏移生成分布。
...
denoising diffusion as stochastic generative process。这篇论文提供的是一个可迁移的结构化 bias：它把 VLA/模仿学习依赖人类 demonstrations，但人类示范常常覆盖窄、动作保守、失败边界少；直接 RL 又容易在高维动作空间中不稳定。 转化为可建模、可采样或可约束的问题。

### 与 [[EmbodiedAI]] 的联系
visuomotor policy and manipulation。这篇论文提供的是一个可迁移的结构化 bias：它把 VLA/模仿学习依赖人类 demonstrations，但人类示范常常覆盖窄、动作保守、失败边界少；直接 RL 又容易在高维动作空间中不稳定。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
policy adaptation / RL refinement。这篇论文提供的是一个可迁移的结构化 bias：它把 VLA/模仿学习依赖人类 demonstrations，但人类示范常常覆盖窄、动作保守、失败边界少；直接 RL 又容易在高维动作空间中不稳定。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[Beyond Human Demonstrations- Diffusion-Based Reinforcement Learning to Generate Data for VLA Training.pdf]]
- 项目入口：[[Final_WMTS]]
