...
> [!abstract] 核心贡献
> DayDreamer 证明 Dreamer 可以直接在真实机器人上在线学习：world model 的意义不是替代真实世界，而是用少量真实数据构造足够可控的 imagination。
...
它在当前知识库中的角色是：WMTS 可以把 DayDreamer 作为真机闭环模板：真实 hand 数据进入 replay，先更新 actuator/contact WM，再让 scheduler 在 imagination 中筛选动作。

## 1. 问题设定与动机

### 1.1 一句话核心
真实机器人 RL 受限于试错成本、安全和 reset；仿真预训练又带来 sim-to-real gap。
...
### 1.4 Delta 分析
如果 RSSM 的 latent 能融合 proprioception/vision 并预测短期 reward，actor-critic 可以主要在 imagination 中更新，真实机器人只负责校正 replay buffer。
...
### 2.3 论文核心机制无跳步推导
- 真实机器人并行收集 replay。
- world model 在 off-policy 序列上监督学习。
- actor-critic 在 latent rollout 上优化，再部署到真实机器人继续采样。
...
2. 方法引入结构性假设：如果 RSSM 的 latent 能融合 proprioception/vision 并预测短期 reward，actor-critic 可以主要在 imagination 中更新，真实机器人只负责校正 replay buffer。
...
### 3.2 关键结果与证据
论文报告 A1 在约 1 小时真机数据内学会从翻倒到站立和行走，并能在受到推动后约 10 分钟内适应。

- PDF 线索：Abstract: To solve tasks in complex environments, robots need to learn from
- PDF 线索：complexity of the real world, is prone to simulator inaccuracies, and the resulting
- PDF 线索：within a learned world model, outperforming pure reinforcement learning in video
- PDF 线索：and directly in the real world, without any simulators. Dreamer trains a quadruped
- PDF 线索：1 hour. We then push the robot and find that Dreamer adapts within 10 minutes to
- PDF 线索：hyperparameters across all experiments, we find that Dreamer is capable of online

### 3.3 Ablation 因果链
若不用 world model imagination，真实样本量会成为瓶颈；若只用仿真，现实复杂性和执行器误差无法自动适配。
...
### 4.3 什么时候会失效
DayDreamer 的接触复杂度低于灵巧手；转笔任务需要更强的接触状态估计和失败恢复。
...
WMTS 可以把 DayDreamer 作为真机闭环模板：真实 hand 数据进入 replay，先更新 actuator/contact WM，再让 scheduler 在 imagination 中筛选动作。
...
model-based RL / imagination rollout。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人 RL 受限于试错成本、安全和 reset；仿真预训练又带来 sim-to-real gap。 转化为可建模、可采样或可约束的问题。

### 与 [[StochasticProcess]] 的联系
latent transition and uncertainty。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人 RL 受限于试错成本、安全和 reset；仿真预训练又带来 sim-to-real gap。 转化为可建模、可采样或可约束的问题。

### 与 [[EmbodiedAI]] 的联系
robot learning loop。这篇论文提供的是一个可迁移的结构化 bias：它把 真实机器人 RL 受限于试错成本、安全和 reset；仿真预训练又带来 sim-to-real gap。 转化为可建模、可采样或可约束的问题。
