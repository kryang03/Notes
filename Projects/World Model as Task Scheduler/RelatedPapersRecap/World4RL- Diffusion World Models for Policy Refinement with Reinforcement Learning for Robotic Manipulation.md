...
> [!abstract] 核心贡献
> World4RL 用 diffusion world model 支持 RL policy refinement：生成式 dynamics 不只是补数据，而是为策略改进提供可采样的未来分布。
...
它在当前知识库中的角色是：WMTS 可将 diffusion WM 用于 contact outcome distribution，帮助 scheduler 选择风险更低的动作 chunk。

## 1. 问题设定与动机

### 1.1 一句话核心
传统 deterministic world model 容易在多模态接触后果上平均；RL 真机微调又成本高。
...
### 1.4 Delta 分析
Diffusion model 能表示 action-conditioned future 的多模态分布，让 policy refinement 在更真实的不确定未来上训练。
...
### 2.3 论文核心机制无跳步推导
- 训练 diffusion world model 预测未来状态/轨迹。
- 在生成的 rollout 中评估策略或构造 RL 更新。
- 用真实数据校正模型并筛选高风险生成。
...
2. 方法引入结构性假设：Diffusion model 能表示 action-conditioned future 的多模态分布，让 policy refinement 在更真实的不确定未来上训练。
...
- A. Overall Framework
- 1 We use x0 to denote the original observation for consistency with diffusion
- V. A BLATION S TUDY

### 3.2 关键结果与证据
关注 policy refinement 后 manipulation 成功率、模型 rollout 质量和 diffusion WM 消融。

- PDF 线索：Abstract—Robotic manipulation policies are commonly ini- inevitably introduces discrepancies from real-world physics,
- PDF 线索：in real-world simulation, with diffusion models in particular opens new opportunities for modeling complex and dynamic
- PDF 线索：frozen world model to avoid online real-world interactions.
- PDF 线索：improve modeling fidelity. Extensive simulation and real-world that predicts future observations conditioned on current ob-
- PDF 线索：environment modeling and enables consistent policy refinement, sparse success signals, enabling policy optimization without
- PDF 线索：yielding significantly higher success rates compared to imitation

### 3.3 Ablation 因果链
deterministic WM -> 多模态后果被平均；无 RL refinement -> 只会模仿旧数据；二者结合改善策略。
...
### 4.3 什么时候会失效
生成式 WM 可能产生看似合理但物理不一致的未来；必须加物理约束或真实一致性检查。
...
WMTS 可将 diffusion WM 用于 contact outcome distribution，帮助 scheduler 选择风险更低的动作 chunk。
...
denoising diffusion as stochastic generative process。这篇论文提供的是一个可迁移的结构化 bias：它把 传统 deterministic world model 容易在多模态接触后果上平均；RL 真机微调又成本高。 转化为可建模、可采样或可约束的问题。

### 与 [[EmbodiedAI]] 的联系
visuomotor policy and manipulation。这篇论文提供的是一个可迁移的结构化 bias：它把 传统 deterministic world model 容易在多模态接触后果上平均；RL 真机微调又成本高。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
policy adaptation / RL refinement。这篇论文提供的是一个可迁移的结构化 bias：它把 传统 deterministic world model 容易在多模态接触后果上平均；RL 真机微调又成本高。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[World4RL- Diffusion World Models for Policy Refinement with Reinforcement Learning for Robotic Manipulation.pdf]]
- 项目入口：[[Final_WMTS]]
