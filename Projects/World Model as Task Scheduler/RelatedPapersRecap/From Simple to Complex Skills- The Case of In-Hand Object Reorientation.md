...
> [!abstract] 核心贡献
> 这篇论文把手内重定向从简单技能逐步推到复杂技能，核心思想是先学稳定可复用的局部旋转/接触技能，再组合成长 horizon 操作。
...
它在当前知识库中的角色是：WMTS 的 Task Scheduler 本质上就是技能组合器：转笔可拆成推进、换指、飞行、捕获、稳定几个可验证子技能。

## 1. 问题设定与动机

### 1.1 一句话核心
直接训练任意姿态重定向会遇到稀疏成功、接触模式多、探索空间大和失败不可恢复。
...
### 1.4 Delta 分析
复杂 dexterous skill 可以由一组更简单、更稳定的技能片段组成；课程或层次结构能降低探索难度。
...
### 2.3 论文核心机制无跳步推导
- 定义简单子任务，例如绕单轴旋转、保持抓取、局部姿态校正。
- 训练或复用基础技能。
- 通过课程、调度器或层次策略组合技能完成复杂重定向。
...
2. 方法引入结构性假设：复杂 dexterous skill 可以由一组更简单、更稳定的技能片段组成；课程或层次结构能降低探索难度。
...
### 3.2 关键结果与证据
关注从简单到复杂课程带来的成功率提升、训练稳定性和未见目标泛化。

- PDF 线索：them to the real world has become a promising approach their first serve: tossing a ball into the air, swinging their
- PDF 线索：complex tasks. Specifically, we introduce a hierarchical policy it improves with practice.
- PDF 线索：low-level skill to execute based on feedback from both the that learning robot behavior for new tasks should leverage ex-
- PDF 线索：simulation to real-world environments. Additionally, we propose
- PDF 线索：inputs to estimate the object’s pose over time. We demonstrate object reorientation using pre-trained object rotation skills
- PDF 线索：ing and then transfer it to the real world [4], [5], [6]. and reinforcement learning [12]. However, it often suffers

### 3.3 Ablation 因果链
无课程端到端训练 -> 探索难且失败早；只用简单技能不组合 -> 无法覆盖复杂目标。
...
### 4.3 什么时候会失效
手工技能分解若不对应真实接触相位，会让 scheduler 在错误边界切换。
...
WMTS 的 Task Scheduler 本质上就是技能组合器：转笔可拆成推进、换指、飞行、捕获、稳定几个可验证子技能。
...
contact mode, friction, grasp stability。这篇论文提供的是一个可迁移的结构化 bias：它把 直接训练任意姿态重定向会遇到稀疏成功、接触模式多、探索空间大和失败不可恢复。 转化为可建模、可采样或可约束的问题。

### 与 [[Dynamics]] 的联系
hand-object rigid body dynamics。这篇论文提供的是一个可迁移的结构化 bias：它把 直接训练任意姿态重定向会遇到稀疏成功、接触模式多、探索空间大和失败不可恢复。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
policy learning under contact。这篇论文提供的是一个可迁移的结构化 bias：它把 直接训练任意姿态重定向会遇到稀疏成功、接触模式多、探索空间大和失败不可恢复。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[From Simple to Complex Skills- The Case of In-Hand Object Reorientation.pdf]]
- 项目入口：[[Final_WMTS]]
