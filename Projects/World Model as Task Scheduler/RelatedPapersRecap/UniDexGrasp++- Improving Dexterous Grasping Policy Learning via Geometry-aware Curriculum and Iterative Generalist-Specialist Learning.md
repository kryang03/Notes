...
> [!abstract] 核心贡献
> UniDexGrasp++ 把几何感知课程和通才-专才迭代结合，用于大规模灵巧抓取策略学习。
...
它在当前知识库中的角色是：WMTS 的任务生成器可按动力学/接触难度排序，而不是随机给转笔目标；难 phase 用 specialist 攻克。

## 1. 问题设定与动机

### 1.1 一句话核心
灵巧抓取的物体几何多样、接触可行性复杂；直接多物体训练容易被难例拖垮或被易例主导。
...
### 1.4 Delta 分析
先按几何难度组织课程，再用 generalist-specialist 循环处理难物体，可以同时提升覆盖和精度。
...
### 2.3 论文核心机制无跳步推导
- 根据物体几何/抓取难度构造课程。
- 训练 generalist policy 覆盖广泛物体。
- 为难物体训练 specialist，再迭代蒸馏或增强 generalist。
...
2. 方法引入结构性假设：先按几何难度组织课程，再用 generalist-specialist 循环处理难物体，可以同时提升覆盖和精度。
...
### 3.2 关键结果与证据
关注大规模物体集抓取成功率、未见物体泛化、课程和 specialist 消融。

- PDF 线索：UniDexGrasp[69], UniDexGrasp++ is trained on 3000+ different object instances with random object poses under a table-top
- PDF 线索：setting. It significantly outperforms the previous SOTA and achieves 85.4% and 78.2% success rates on the train and test set.
- PDF 线索：mation under a table-top setting, namely UniDexGrasp++. works [61, 6, 17, 23, 65, 16, 12] have made significant
- PDF 线索：ous grasping on thousands of object instances with 85.4%
- PDF 线索：and 78.2% success rate on the train set and test set which
- PDF 线索：outperforms the state-of-the-art baseline UniDexGrasp by

### 3.3 Ablation 因果链
无几何课程 -> 训练不稳定；无 specialist -> 难例提升有限；无 generalist -> 泛化不足。
...
### 4.3 什么时候会失效
抓取成功不等于动态操作成功；转笔需要在稳定抓取之外建模释放和再捕获。
...
WMTS 的任务生成器可按动力学/接触难度排序，而不是随机给转笔目标；难 phase 用 specialist 攻克。
...
contact mode, friction, grasp stability。这篇论文提供的是一个可迁移的结构化 bias：它把 灵巧抓取的物体几何多样、接触可行性复杂；直接多物体训练容易被难例拖垮或被易例主导。 转化为可建模、可采样或可约束的问题。

### 与 [[Dynamics]] 的联系
hand-object rigid body dynamics。这篇论文提供的是一个可迁移的结构化 bias：它把 灵巧抓取的物体几何多样、接触可行性复杂；直接多物体训练容易被难例拖垮或被易例主导。 转化为可建模、可采样或可约束的问题。

### 与 [[ReinforcementLearning]] 的联系
policy learning under contact。这篇论文提供的是一个可迁移的结构化 bias：它把 灵巧抓取的物体几何多样、接触可行性复杂；直接多物体训练容易被难例拖垮或被易例主导。 转化为可建模、可采样或可约束的问题。

## References
- 原始 PDF：[[UniDexGrasp++- Improving Dexterous Grasping Policy Learning via Geometry-aware Curriculum and Iterative Generalist-Specialist Learning.pdf]]
- 项目入口：[[Final_WMTS]]
